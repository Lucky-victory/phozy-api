  ...
  static async createNewUser(req: Request, res: Response): Promise<void> {
    try {
      let username = req.body.username
        ? req.body.username
        : randomWords({ exactly: 2, maxLength: 6 });
      ...
      const [usernameExist, emailExist] = await Promise.all([
        await UsersModel.findByUsername(newUser.username),
        UsersModel.findByEmail(newUser.email),
      ]);
      ...
      try {
        const result = (await UsersModel.findById(insertId[0])) as IUsersResult;
        user = result;
